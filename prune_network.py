...
parser.add_argument('--threshold',
                    type=float,
                    default=0.05,
                    help='the threshold for pruning')
...
def main():
	...
	po_loss, po_acc = test(model=net,
	                       criterion=criterion,
	                       data_loader=poison_test_loader)
	cl_loss, cl_acc = test(model=net,
	                       criterion=criterion,
	                       data_loader=clean_test_loader)
	print('0 \t None     \t None     \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f}'.
	      format(po_loss, po_acc, cl_loss, cl_acc))
	results = evaluate_by_threshold(net,
	                                mask_values,
	                                criterion=criterion,
	                                clean_loader=clean_test_loader,
	                                poison_loader=poison_test_loader)
	file_name = os.path.join(args.output_dir,
	                         'pruning_by_{}.txt'.format(args.threshold))
...
def evaluate_by_threshold(model, mask_values, criterion, clean_loader,
                          poison_loader):
	results = []
	start = 0
	idx = start
	for idx in range(start, len(mask_values)):
		if float(mask_values[idx][2]) <= args.threshold:
			...
			break
	layer_name, neuron_idx, value = mask_values[idx][0], mask_values[idx][
	    1], mask_values[idx][2]
	cl_loss, cl_acc = test(model=model,
	                       criterion=criterion,
	                       data_loader=clean_loader)
	po_loss, po_acc = test(model=model,
	                       criterion=criterion,
	                       data_loader=poison_loader)
	print('{:.2f} \t {} \t {} \t {} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f}'.
	      format(start, layer_name, neuron_idx, args.threshold, po_loss,
	             po_acc, cl_loss, cl_acc))
	results.append(
	    '{:.2f} \t {} \t {} \t {} \t {:.4f} \t {:.4f} \t {:.4f} \t {:.4f}\n'.
	    format(start, layer_name, neuron_idx, args.threshold, po_loss, po_acc,
	           cl_loss, cl_acc))
	return results
